...
DailySeriesTrimmerProcess () {
    ...
    for id in $(echo $sonarrSeriesIds); do
        ...
        if [ $seriesType != "daily" ]; then
            maximumDailyEpisodes=$(echo "$seriesData" | jq -r "[.seasons | map(select(.seasonNumber > 0 and .statistics.totalEpisodeCount > 1)) | sort_by(.seasonNumber) | reverse | .[:2] | .[].statistics.totalEpisodeCount] | add")
        fi
